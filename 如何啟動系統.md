# 🚀 如何啟動/重啟交易系統（marketview_En — 本機專用）

**本專案**：美股、台股、國際、重金屬、財報日由 Yahoo Finance；**加密貨幣由 Deribit 交易所**取得，皆無需 API key，專供本機使用。
...
#### 步驟 1：打開終端機
有幾種方式：
1. **PowerShell**：按 `Win + X` → 「終端機」，或 `Win + R` → `powershell`
2. **CMD**：`Win + R` → `cmd`
3. **檔案總管**：進入 `D:\marketview_En`，地址欄輸入 `powershell` 或 Shift+右鍵「在此處開啟 PowerShell」
...
**本專案（marketview_En）**：全部使用 **yfinance**，不需任何 API key，適合本機使用。若需對外或雲端部署，請使用另一套 `trading_system`（多資料源版）。
