...
class Config:
    ...
    # 加密貨幣專區（Deribit 交易所，24 小時交易；其餘為 yfinance）
    CRYPTO = {
        'BTC-USD': '比特幣',
        'ETH-USD': '以太幣',
        'BNB-USD': 'BNB',
    ...
    # Benchmark 績效試算：輸入期間後計算各指數期間報酬（幾點漲到幾點）
    BENCHMARK_INDICES = {
        '^GSPC': 'S&P 500',
        '^IXIC': '那斯達克',
        '^N225': '日經225',
    ...
    # 本專案（marketview_En）美股/台股/重金屬等用 yfinance；加密貨幣用 Deribit
    # 財報行事曆：Finnhub、FMP 為選填備援（未設則用 yfinance + calendar fallback）
    FINNHUB_API_KEY = os.environ.get('FINNHUB_API_KEY', '').strip()
    FMP_API_KEY = os.environ.get('FMP_API_KEY', '').strip()
    # 選股策略（selecting）：FinMind 財報，免費 600 次/時；可設第二個金鑰擴充額度
