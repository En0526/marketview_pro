...
def get_stock_screener():
    """選股策略1 API：毛利率連續三月>30%且成長 + 近三月營收年成長率>20% + 5MA>20MA（台股上市櫃，數千檔）"""
    from flask import request
    try:
        from selecting.stock_screener import run_screener
        min_margin = float(request.args.get('min_margin', 30))
        min_quarters = int(request.args.get('min_quarters', 3))
        min_revenue_yoy = float(request.args.get('min_revenue_yoy', 20))
        tech_only = request.args.get('tech_only', 'true').lower() == 'true'
        ...
        return jsonify({
            ...
            'data': {
                ...
                'criteria': {
                    'gross_margin_min_pct': min_margin,
                    'consecutive_quarters': min_quarters,
                    'revenue_yoy_min_pct': min_revenue_yoy,
                    'ma_condition': '5MA > 20MA',
                    'tech_only': tech_only,
                },
...
def get_benchmark_performance():
    """Benchmark 績效試算：輸入 start_date、end_date（YYYY-MM-DD），回傳各指數期間報酬與幾點漲到幾點"""
    from flask import request
    try:
        ...
        if not start_date or not end_date:
            return jsonify({
                'success': False,
                'error': '請輸入起始日與結束日（格式：YYYY-MM-DD）',
                'data': None,
            }), 400
        data = data_fetcher.get_benchmark_performance(start_date, end_date)
        if data.get('error'):
            return jsonify({
                'success': False,
                'error': data['error'],
