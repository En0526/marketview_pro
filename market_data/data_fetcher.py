...
from market_data.finnhub_client import get_earnings_calendar as finnhub_get_earnings_calendar
...
class MarketDataFetcher:
    ...
    def get_earnings_calendar(self, days_ahead: int = 60, force_refresh: bool = False) -> Dict[str, Dict]:
        ...
        try:
            ...
            if api_key:
                result = finnhub_get_earnings_calendar(api_key, from_str, to_str, Config.US_STOCKS)
                if result:
            ...
            if fmp_key:
                result = fmp_get_earnings_calendar(fmp_key, from_str, to_str, Config.US_STOCKS)
                if result:
                    self._earnings_cache = result
                    self._earnings_cache_time = now
                    return result
    ...
    def get_benchmark_performance(self, start_date: str, end_date: str) -> Dict:
        """
        取得指定期間內各 benchmark 指數的績效：起始點、結束點、期間報酬%。
        ...
        try:
            ...
            return {'error': '日期格式錯誤，請使用 YYYY-MM-DD', 'results': []}
        if start > end:
            return {'error': '起始日不可晚於結束日', 'results': []}
        indices = getattr(Config, 'BENCHMARK_INDICES', {})
        if not indices:
            return {'error': '未設定 BENCHMARK_INDICES', 'results': []}
        results = []
        for symbol, name in indices.items():
            try:
                ticker = yf.Ticker(symbol)
                df = ticker.history(start=start, end=end + pd.Timedelta(days=1), interval='1d')
                if df is None or df.empty or 'Close' not in df.columns:
                    results.append({
                        'symbol': symbol,
                        'name': name,
    ...
    def get_market_summary(self, sections: Optional[List[str]] = None) -> Dict:
        ...
        all_tasks = {
            ...
            'crypto': lambda: deribit_get_multiple(getattr(Config, 'CRYPTO', {})),
